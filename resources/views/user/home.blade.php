...
@section('content')
    ...
    <nav class="w-full py-4 border-t border-b bg-gray-100" x-data="{ open: false }">
        <div class="block sm:hidden">
            <a href="#"
                ...
                @click="open = !open">
                Trending Categories <i :class="open ? 'fa-chevron-down' : 'fa-chevron-up'" class="fas ml-2"></i>
            </a>
        </div>
    ...
    <div class="container mx-auto flex flex-wrap py-6">
        ...
        <aside class="w-full md:w-1/3 flex flex-col items-center px-3">
            ...
            <div class="w-full bg-white shadow flex flex-col my-4 p-6">
                <p class="text-xl font-semibold pb-5">Authors you may like</p>
                <div class="grid grid-cols-3 gap-3">
                    @foreach (get_recent_authors() as $author)
                        <div class="py-6 flex items-center justify-center font-bold text-2xl uppercase {{ generate_random_bg_col_class('bg-', '-400') }}">
                            {{ generate_user_initials($author->first_name, $author->last_name) }}
                        </div>
                    @endforeach
                </div>
                <a href="#"
