@props(['name', 'id', 'placeholder' => '*****', 'traits' => []])
<div class="relative @if ($errors->has($name)) border border-red-500 rounded-md @endif">
    <input type="password" name="{{ $name }}" id="{{ $id }}" placeholder="{{ $placeholder }}"
        @foreach ($traits as $key => $value) {!! $key.'='.$value !!} @endforeach
