@props(['name', 'id', 'placeholder' => 'leroy@jenkins.com', 'label' => '', 'traits' => []])
@if ($label)
    <label for="{{ $name }}" class="block text-sm">{{ $label }}</label>
@endif
<div class=" @if ($errors->has($name)) border rounded-md border-red-500 @endif">
<input @foreach ($traits as $key => $value) {!! $key.'='.$value !!} @endforeach type="email" name="{{ $name }}" id="{{ $id }}" placeholder="{{ $placeholder }}" value="{{ old($name) }}"
